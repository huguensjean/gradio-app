...
including a fix to ensure subscription-topic routing is consistent by assigning
unique subs per topic, and Gemini integration.
Now preserves entire JSON messages for full chat history,
while a separate Dataflow pipeline can filter output messages (user & Gemini
only).
"""
...
def send_message(
  ...
  def background_send():
    """Background logic for sending user message + Gemini response to Pub/Sub."""
    ...
    with chat_lock:
      ...
      # Always publish the user message
      if combined_text:
        msg_data = {
      ...
      # if combined_text and app_name_input != "Gemini":
      # Gemini call
      # file_bytes = None
      # file_mime = None
      # if files:
      ...
      #      file_bytes = f.read()
      #  except Exception as e:
      #    logging.error(f"Error reading file for Gemini: {e}")

      # gemini_response = generate_gemini_response(
      #    combined_text, file_bytes, file_mime, stream=True
      # )
      # gemini_timestamp = datetime.datetime.now().isoformat(timespec="microseconds")
      # gemini_msg_data = {
      #    "sender": "Gemini",
      #    "text": gemini_response,
      #    "timestamp": gemini_timestamp,
      #    "message_id": str(uuid.uuid4()),
      #    "app_name": "Gemini",
      # }
      # success = publish_message_with_retry(
      #    publisher, topic_path, gemini_msg_data, MAX_PUBLISH_RETRIES
      # )
      # if not success:
      #  logging.error("Failed to publish Gemini message after retries.")
      #  return

      # store_message_entire_json(selected_topic, gemini_msg_data)

      # If user is Gemini, just publish that directly
      # elif combined_text and app_name_input == "Gemini":
      # msg_data = {
      #    "sender": app_name_input,
      #    "text": combined_text,
      #    "timestamp": timestamp,
