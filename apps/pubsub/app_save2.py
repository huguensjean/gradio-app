...
from common_utils import create_publisher, create_subscriber, create_subscription, create_topic
from common_utils import project_id, publish_message, pull_messages
import fastapi
from gcp_utils import get_project_id, get_region
...
gemini_client = genai.Client(
    vertexai=True, project=get_project_id(), location=get_region()
)
...
# Global flag for checking if Gemini has replied.
waiting_for_gemini_response = None
...
def get_chat_history_from_db(topic_name: str):
  ...
  try:
    ...
    for doc in docs:
      d = doc.to_dict()
      sender = d["sender"] if "sender" in d else "UNKNOWN"
...
def create_new_subscription(topic_name, subscription_name_input, user_id):
  ...
  unique_subscription_name = f"{subscription_name_input}-{user_id}"
  logging.info(
...
def create_subscription_with_user_id(
  ...
  unique_sub = f"{sub_name_input}-{user_id}"
...
def generate_gemini_response(
    prompt, file_bytes=None, file_mime=None, stream=True
):
  """Generates a response from Gemini with an optional image/audio/video."""
  try:
    input_content = []
    if prompt:
    ...
    if file_bytes and file_mime:
      input_content.append(
          types.Part.from_bytes(
              data=file_bytes, mime_type=file_mime or "application/octet-stream"
          )
      )
    if not input_content:
      logging.warning("No content provided to Gemini API.")
      return "No input provided."

    if stream:
      response_stream = gemini_client.models.generate_content_stream(
          model="gemini-2.0-flash-exp", contents=input_content
      )
      full_response = ""
...
async def _upload_files_to_gcs(files: list) -> str:
  """Uploads files to GCS and returns HTML."""
  combined_text = ""
...
async def send_message(
    message_input, history, app_name_input, selected_topic, selected_subscription
):
    """Sends a message to the selected Pub/Sub topic (including optional files),

    optionally calls Gemini for a reply, and stores messages in Firestore.
    """
    logging.info(
    ...
    chat_history = get_chat_history_from_db(selected_topic)
    chat_history_length_before = len(chat_history)
    logging.info(f"FIRST Chat history before: {chat_history}")
    logging.info(f"BEFORE Chat history length: {chat_history_length_before}")
    logging.info(f"message_input: {message_input}")
    logging.info(f"message_input type: {type(message_input)}")

    # Initialize text and files to empty defaults
    text = ""
    files = []
    
    # Force message_input to always be a dictionary
    if isinstance(message_input, tuple):
        message_input = {"text": "", "files": []} #handles the chatinterface sending a tuple
    if not isinstance(message_input, dict):
        message_input = {"text": "", "files": []} #handles an empty list.
    ...
    if expected_topic != selected_topic:
        ...
        return (
            [{"role":"System", "content":"Error: The chosen subscription does not match the selected topic!"}],
            gr.MultimodalTextbox(value=None, interactive=True),
            gr.Dropdown(
                choices=update_available_topics(),
                value=selected_topic,
    ...
    if not text and not files:
      logging.info("No text and no files, getting history.")

      formatted_history = {}
      for sender, content in chat_history:
        formatted_history.get("role", sender)
        formatted_history.get("content", content)
    ...
    async def background_send():
      ...
      with chat_lock:
          ...
          combined_text += await _upload_files_to_gcs(files)
          # If user is NOT Gemini, we also have Gemini respond
          if "Gemini" not in combined_text and app_name_input != "Gemini":
    ...
    await background_send()  # Ensure background send completes

    # Polling for Firestore to be consistent
    max_retries = 50  # Adjust number of retries as needed
    retry_delay = 0.1  # Adjust retry delay as needed (seconds)

    chat_history = get_chat_history_from_db(selected_topic)
    chat_history_length_after_gemini = len(chat_history)

    for _ in range(max_retries):
        logging.info("Polling for Firestore to be consistent...")
        time.sleep(.1)
        chat_history = get_chat_history_from_db(selected_topic)
        chat_history_length_after = len(chat_history)
        if (
            any("Gemini" in sender for sender, _ in chat_history)
            or "Gemini" not in combined_text
        ):
            if chat_history_length_after > chat_history_length_after_gemini:
                logging.info(f"AFTER Chat history length: {chat_history_length_after}")
                logging.info("Gemini message found in chat history, exiting polling.")
                break
        else:
            logging.info("Gemini message not found in chat history, retrying...")
            await asyncio.sleep(retry_delay)

    logging.warning(
        "Max retries exceeded waiting for Gemini message, using last chat"
        " history. Gemini message may not be available."
    )

    # Format chat history from db into a list of dictionaries with role and content keys
    formatted_history = {}
    for sender, content in chat_history:
      formatted_history.get("role", sender)
      formatted_history.get("content", content)
...
# def keep_polling_db():
#  """Called by APS#cheduler every second, triggers a UI update by calling poll_updates."""
#  global active_chat_output
#  global active_topic_dropdown
#  global waiting_for_gemini_response
#  logging.info(
#      "keep_polling_db called with active_chat_output:"
#      f" {active_chat_output} and active_topic_dropdown:"
#      f" {active_topic_dropdown}  and waiting_for_gemini_response:"
#      f" {waiting_for_gemini_response}"
#  )
...
with gr.Blocks(css="#chatbot {overflow:auto; height:800px;}") as demo:
    ...
    demo.load(
        fn=set_globals,
        inputs=[chat_interface.chatbot, topic_dropdown],
