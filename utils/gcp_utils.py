...
def authenticate(user_email, access_token):
  """Check if a user has access to the project."""
  if check_user_access_with_email(
      user_email, PROJECT_ID
  ) and check_user_access_with_token(PROJECT_ID, access_token):
    return True


def get_access_token():
  """Get the access token from the metadata service."""
...
def check_user_access_with_email(user_email, project_id):
  ...
  for binding in policy.bindings:
    if f"user:{user_email}" in binding.members:
      print(f"User {user_email} has role(s): {binding.role}")
      return True

  print(f"User {user_email} does NOT have explicit roles on {resource_name}.")
  return False
...
def check_user_access_with_token(project_id, access_token):
  """Check if a user has access to a project."""
  user_email = get_user_email_from_access_token(access_token)
  if not user_email:
    print("Could not retrieve user email from access token.")
    return False
